...
var JqueryDialog = {
	...
	openPanel:function( iframeSrc, iframeWidth, iframeHeight){
		JqueryDialog.init('', iframeSrc, iframeWidth, iframeHeight, false, false, false);
	},
	...
	init:function(dialogTitle, iframeSrc, iframeWidth, iframeHeight, isSubmitButton, isCancelButton, isDrag){
		...
		if(typeof($("#jd_shadow")[0]) == "undefined"){
			//前置
			$("body").prepend("<div oncontextmenu='return false' id='jd_shadow'>&nbsp;</div>");
		...
		var _top = (document.documentElement.clientHeight - (iframeHeight + JqueryDialog.cBorderSize * 2  + 5)) / 2;
		_jd_dialog.css("top", (_client_height - iframeHeight)/2 + "px");
		...
		//var _jd_dialog_m_h = jq("#jd_dialog_m_h");
		...
		//var _jd_dialog_m_t = jq("#jd_dialog_m_t");
		//if(isSubmitButton){
	...
	/// <summary>关闭模态窗口</summary>
	closePanel:function(resultName,resultVal){
		parent.$("#jd_shadow").remove();
		parent.$("#jd_dialog").remove();
	},
	...
	resize : function (width,height){
		$(_jd_dialog[0]).css("width", width + "px");
		$(_jd_dialog_m_h[0]).css("height",height + "px");
	}
};
...
var DragAndDrop = function(){
	...
	//	jq(_dragObj).css("left", _dragObjCurrentLocation.x + "px");
	//	jq(_dragObj).css("top", _dragObjCurrentLocation.y + "px");
	//};
